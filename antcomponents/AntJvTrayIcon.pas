...
type
  ...
  TAntJvTrayIcon = class(TComponent)
  private
    FActive: Boolean;
    FIcon: TIcon;
...
constructor TAntJvTrayIcon.Create(AOwner: TComponent);
begin
...
destructor TAntJvTrayIcon.Destroy;
begin
  UnHook;
...
function TAntJvTrayIcon.AcceptBalloons: Boolean;
begin
  //Balloons are only accepted with shell32.dll 5.0+
  Result := GetShellVersion >= $00050000;
end;

procedure TAntJvTrayIcon.SetTask(const Value: Boolean);
begin
  if FTask <> Value then
  begin
    FTask := Value;
...
procedure TAntJvTrayIcon.WndProc(var Mesg: TMessage);
var
  I: Integer;
  Pt: TPoint;
  ShState: TShiftState;
...
procedure TAntJvTrayIcon.IconChanged(Sender: TObject);
begin
  DoCheckCrash;
  with FIc do
    hIcon := FIcon.Handle;
...
procedure TAntJvTrayIcon.SetHint(Value: string);
begin
  DoCheckCrash;
...
procedure TAntJvTrayIcon.SetIcon(Icon: TIcon);
begin
  DoCheckCrash;
  FIcon.Assign(ICon);
  with FIc do
...
procedure TAntJvTrayIcon.SetActive(Value: Boolean);
...
procedure TAntJvTrayIcon.SetApplicationVisible(Value: Boolean);
begin
...
procedure TAntJvTrayIcon.HideApplication;
begin
...
procedure TAntJvTrayIcon.ShowApplication;
begin
...
procedure TAntJvTrayIcon.OnAnimateTimer(Sender: TObject);
begin
...
procedure TAntJvTrayIcon.SetAnimated(const Value: Boolean);
begin
  FAnimated := Value;
...
procedure TAntJvTrayIcon.SetDelay(const Value: Cardinal);
begin
  FDelay := Value;
  if FTimer <> nil then
    FTimer.Interval := FDelay;
...
procedure TAntJvTrayIcon.SetImgList(const Value: TImageList);
begin
  FImgList := Value;
end;

procedure TAntJvTrayIcon.BalloonHint(Title, Value: string;
...
procedure TAntJvTrayIcon.DoCheckCrash;
var
  HWndTray: HWND;
begin
  if Active then
...
procedure TAntJvTrayIcon.DoMouseMove(Shift: TShiftState; X, Y: Integer);
begin
...
procedure TAntJvTrayIcon.DoMouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TAntJvTrayIcon.DoMouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TAntJvTrayIcon.DoDoubleClick(Button: TMouseButton;
...
procedure TAntJvTrayIcon.SetNumber(const Value: Integer);
var
  Ico: TIcon;
begin
  FNumber := Value;
...
procedure TAntJvTrayIcon.SetVisibility(const Value: TTrayVisibilities);
begin
  FVisibility := Value;
...
function TAntJvTrayIcon.ApplicationHook(var Msg: TMessage): Boolean;
begin
...
procedure TAntJvTrayIcon.UnHook;
begin
  if not FHooked then
    Exit;
