...
type
  ...
  TAntJvListItem = class(TListItem)
  private
  ...
  TAntJvListView = class(TListView)
  private
...
constructor TAntJvListItem.CreateEnh(AOwner: TListItems);
begin
...
procedure TAntJvListItem.SetPopupMenu(const Value: TPopupMenu);
begin
  FPopupMenu := Value;
end;
...
constructor TAntJvListView.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FHintColor := clInfoBk;
  FOver := False;
...
procedure TAntJvListView.WMNotify(var Msg: TWMNotify);
var
  Node: TListItem;
  Point: TPoint;
begin
      ...
      NM_CLICK, NM_RCLICK:
        begin
          ...
          if (Selected <> nil) and (NMHDR^.code = NM_RCLICK) then
            TAntJvListItem(Selected).PopupMenu.Popup(Mouse.CursorPos.X, Mouse.CursorPos.Y);
        end;
    end;
  end;
end;

procedure TAntJvListView.WMHScroll(var Msg: TWMHScroll);
begin
  inherited;
...
procedure TAntJvListView.WMVScroll(var Msg: TWMVScroll);
begin
  inherited;
...
procedure TAntJvListView.MouseEnter(var Msg: TMessage);
begin
  FOver := True;
...
procedure TAntJvListView.MouseLeave(var Msg: TMessage);
begin
...
procedure TAntJvListView.CMCtl3DChanged(var Msg: TMessage);
begin
  inherited;
...
procedure TAntJvListView.CMParentColorChanged(var Msg: TMessage);
begin
  inherited;
...
procedure TAntJvListView.ColClick(Column: TListColumn);
type
  TParamSort = record
    Index: Integer;
    Sender: TObject;
...
var
  ...
  begin
    ...
    if Assigned(TAntJvListView(Parm.Sender).FOnAutoSort) then
      SortKind := TAntJvListView(Parm.Sender).FOnAutoSort(Parm.Sender, Parm.Index)
    else
...
function TAntJvListView.CreateListItem: TListItem;
begin
  Result := TAntJvListItem.CreateEnh(Items);
end;

function TAntJvListView.GetItemPopup(Node: TListItem): TPopupMenu;
begin
  Result := TAntJvListItem(Node).PopupMenu;
end;

procedure TAntJvListView.SetItemPopup(Node: TListItem; Value: TPopupMenu);
begin
  TAntJvListItem(Node).PopupMenu := Value;
end;

procedure TAntJvListView.LoadFromFile(FileName: string);
var
  Stream: TFileStream;
begin
...
procedure TAntJvListView.LoadFromStream(Stream: TStream);
var
...
procedure TAntJvListView.SaveToFile(FileName: string; ForceOldStyle: Boolean);
var
  Stream: TFileStream;
begin
...
procedure TAntJvListView.SaveToStream(Stream: TStream; ForceOldStyle: Boolean);
...
procedure TAntJvListView.LoadFromCSV(FileName: string; Separator: Char);
var
  st, st2: string;
  fich: textfile;
...
procedure TAntJvListView.SaveToCSV(FileName: string; Separator: Char);
var
  st: string;
  fich: textfile;
  I, J: Integer;
...
procedure TAntJvListView.InvertSelection;
var
  I: Integer;
begin
  Items.BeginUpdate;
...
procedure TAntJvListView.UnselectAll;
var
  I: Integer;
begin
  Items.BeginUpdate;
...
procedure TAntJvListView.KeyUp(var Key: Word; Shift: TShiftState);
var
  st: string;
  I, J: Integer;
begin
...
function TAntJvListView.GetColumnsOrder: string;
var
...
begin
  ...
  if Columns.Count > cColumnsHandled then
    raise EJvListViewError.Create('TAntJvListView.GetColumnsOrder: too many columns');
  for I := 0 to Columns.Count - 1 do
  begin
    if Result <> '' then
      Result := Result + ',';
...
procedure TAntJvListView.SetColumnsOrder(const Order: string);
var
  Res: array [0..cColumnsHandled-1] of Integer;
  I, J: Integer;
  st: string;
