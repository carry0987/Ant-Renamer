...
type
  TAntJvZoom = class(TCustomControl)
  private
    FTimer: TTimer;
    FActive: Boolean;
    FZoomLevel: Integer;
...
constructor TAntJvZoom.Create(AOwner: TComponent);
begin
...
procedure TAntJvZoom.FlushCache;
begin
  FreeAndNil(FCacheBitmap);
end;

procedure TAntJvZoom.Loaded;
begin
  inherited Loaded;
  FTimer.Enabled := FActive;
end;

procedure TAntJvZoom.Paint;
begin
  if Active then
    PaintZoom
  else
...
procedure TAntJvZoom.PaintMe(Sender: TObject);
begin
...
procedure TAntJvZoom.PaintZoom;
var
  P: TPoint;
  X, Y, Dx, Dy: Integer;
  SourceRect: TRect;
...
procedure TAntJvZoom.SetActive(const Value: Boolean);
begin
...
procedure TAntJvZoom.SetCacheOnDeactivate(const Value: Boolean);
begin
...
procedure TAntJvZoom.SetDelay(const Value: Cardinal);
begin
  FDelay := Value;
  FTimer.Interval := Value;
end;

procedure TAntJvZoom.WMSize(var Msg: TWMSize);
begin
